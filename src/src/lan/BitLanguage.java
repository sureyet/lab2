...
//  模拟两个策略对抗N回合后的结果
  public static void oppose(int N, String path1, String path2, HashMap<String,Integer> hmap) throws IOException, ParseException {
    //策略1
    FileInputStream fileInputStream1 = new FileInputStream(new File(path1));
    BitLanguage bitLanguage1 = new BitLanguage(fileInputStream1);
    //策略2
    FileInputStream fileInputStream2 = new FileInputStream(new File(path2));
    BitLanguage bitLanguage2 = new BitLanguage(fileInputStream2);

    for (int i = 0;i<N;i++){
      System.out.println("回合: "+i);
      //回合i下两个策略的结果
      bitLanguage1.analyse();
      System.out.println("策略1");
      bitLanguage1.valueStack.showValue();
      bitLanguage2.analyse();
      System.out.println("策略2");
      bitLanguage2.valueStack.showValue();
      //设置对方上一个回合的结果
      bitLanguage1.valueStack.setEnemy(bitLanguage2.valueStack.returnValue);
      bitLanguage2.valueStack.setEnemy(bitLanguage1.valueStack.returnValue);

      //重新载入，准备下个回合
      fileInputStream1.close();
      fileInputStream1 = new FileInputStream(new File(path1));
      bitLanguage1.ReInit(fileInputStream1);

      fileInputStream2.close();
      fileInputStream2 = new FileInputStream(new File(path2));
      bitLanguage2.ReInit(fileInputStream2);
    }
    System.out.println(bitLanguage1.valueStack.sum);
    System.out.println(bitLanguage2.valueStack.sum);

    int sum = 0;
    String f = new File(path1).getName();
    sum = hmap.get(f)+ bitLanguage1.valueStack.sum;
    hmap.put(f,sum);
    f = new File(path2).getName();
    sum = hmap.get(f) + bitLanguage2.valueStack.sum;
    hmap.put(f,sum);
  }
